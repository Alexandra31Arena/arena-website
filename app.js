...
/* ===== Add FOR AGENTS tagline under wordmark ===== */
(function(){
  try{
    document.querySelectorAll('.logo').forEach(function(el){
      if(el.querySelector('.logo-tagline')) return;
      var word=el.querySelector('.word');
      var wf=word?parseFloat(getComputedStyle(word).fontSize):18;
      var row=document.createElement('span');
      row.style.display='inline-flex'; row.style.alignItems='center';
      while(el.firstChild){ row.appendChild(el.firstChild); }
      var tag=document.createElement('span');
      tag.className='logo-tagline'; tag.textContent='FOR AGENTS';
      var fs=Math.max(8, wf*0.26);
      tag.style.cssText='display:block;text-align:center;color:#C9A227;font-weight:700;line-height:1;margin-top:'+(fs*0.35)+'px;font-size:'+fs+'px;letter-spacing:'+(fs*0.34)+'px;padding-left:'+(fs*0.34)+'px';
      el.appendChild(row); el.appendChild(tag);
      el.style.display='inline-flex'; el.style.flexDirection='column'; el.style.alignItems='center'; el.style.lineHeight='1';
    });
  }catch(e){}
})();
